...
int main(int argc, char* argv[])
{
	...
	writeDataset(dataSetSize, outputFileName, ds, avg, min, max, bufferSize);
...
void selectionSort(float* ds, int n)
{
	int limitCheckInterval = (n-1)/10;	//print every 10%
	int printer = 0;
	...
	{ // Find the minimum element in unsorted array 
		...
		if(!(i%limitCheckInterval)){
			std::cout << "selectionSort() " << printer << "%" << std::endl;
			printer += 10;
		}
...
int writeDataset(int dataSetSize, char* filename, float* ds, int bufferSize, float avg, float min, float max)
{
	...
	for (int i = 0; i < dataSetSize/ bufferSize; i+= bufferSize)
	{
		if(!(i%limitCheckInterval)){
		...
		fwrite(&ds[i], sizeof(float), bufferSize, fp);
	}
	fclose(fp);
